...
async function fetchStrapi(params : any) {
	...
	return await fetch("https://webmaster.dequity.io/api/marketplace-page?locale="+params.lang, options)
...
import {
	setMarketplaceFailedAction,
    setMarketplaceFetchLoadingAction,
    setMarketplaceSuccess
} from "../actions/app";

import {MARKETPLACE_FETCH} from "../actions/types";
// import {takeEvery} from "@redux-saga/core/types/effects";

function* asyncfetchCollectionNFTs(params : any): Generator<any> {
	yield put(setMarketplaceFetchLoadingAction());

	try {
		const strapiData = yield call(fetchStrapi, params.filters);

		yield put(setMarketplaceSuccess(strapiData));
	} catch (e) {
		yield put(setMarketplaceFailedAction(e));
	}
}

export default function* fetchMarketplaceSaga() {
	yield takeLatest(MARKETPLACE_FETCH, asyncfetchCollectionNFTs);
}
