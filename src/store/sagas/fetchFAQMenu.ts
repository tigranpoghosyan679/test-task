import * as Effects from "redux-saga/effects";

const call: any = Effects.call;
const put: any = Effects.put;
const takeLatest = Effects.takeLatest;


async function fetchStrapi(params : any) {

	// GET request using fetch inside useEffect React hook
	const token = "";
	const options = {
	   method: 'GET',
	   headers: {
		 'Content-type': 'application/json',
		 'Authorization': `bearer ${token}`, // notice the Bearer before your token
		},
	 };
	return await fetch("https://webmaster.dequity.io/api/fa-qs-menus?populate=*&locale="+params.lang, options)
	  .then((response) => response.json())
	  .catch((e)=>{throw e})
	// empty dependency array means this effect will only run once (like componentDidMount in classes)

}
import {
	setFAQsMenuFailedAction,
    setFAQsMenuFetchLoadingAction,
    setFAQsMenuSuccess
} from "../actions/app";

import {FAQSMenu_FETCH} from "../actions/types";
// import {takeEvery} from "@redux-saga/core/types/effects";

function* asyncfetchCollectionNFTs(params : any): Generator<any> {
	yield put(setFAQsMenuFetchLoadingAction());

	try {
		const strapiData = yield call(fetchStrapi, params.filters);

		yield put(setFAQsMenuSuccess(strapiData));
	} catch (e) {
		yield put(setFAQsMenuFailedAction(e));
	}
}

export default function* fetchFAQsMenuSaga() {
	yield takeLatest(FAQSMenu_FETCH, asyncfetchCollectionNFTs);
}
