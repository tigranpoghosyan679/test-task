...
async function fetchStrapi(params : any) {
	...
	return await fetch("https://webmaster.dequity.io/api/fa-qs-p-age?locale="+params.lang, options)
...
import {
	setFAQsFailedAction,
    setFAQsFetchLoadingAction,
    setFAQsSuccess
} from "../actions/app";

import {FAQS_FETCH} from "../actions/types";
// import {takeEvery} from "@redux-saga/core/types/effects";

function* asyncfetchCollectionNFTs(params : any): Generator<any> {
	yield put(setFAQsFetchLoadingAction());

	try {
		const strapiData = yield call(fetchStrapi, params.filters);

		yield put(setFAQsSuccess(strapiData));
	} catch (e) {
		yield put(setFAQsFailedAction(e));
	}
}

export default function* fetchFAQsSaga() {
	yield takeLatest(FAQS_FETCH, asyncfetchCollectionNFTs);
}
