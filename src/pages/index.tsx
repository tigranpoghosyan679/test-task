...
const HomePageNew = dynamic(() => import('../../libs/common/ui/pages/HomePageNew/HomePageNew'));

// import { HomePageNew } from "@common/ui";
...
export const getServerSideProps = async ({ locale } : {locale : any}) => {
  ...
  const resHeader = await fetch("https://webmaster.dequity.io/api/header?locale="+locale, options);
  const dataHeader = await resHeader.json();

  const resFooter = await fetch("https://webmaster.dequity.io/api/footer?locale="+locale, options);
...
export default function Home({data, dataSeo, dataHeader, dataFooter} : {data:any, dataSeo:any, dataHeader: any, dataFooter : any}) {
  ...
  return (
    ...
    <>
      ...
      <HomePageNew strapiData={data} strapiDataHeader={dataHeader} strapiDataFooter={dataFooter} />
